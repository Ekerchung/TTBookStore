        ...
        <c:if test="${not empty requestScope.page.items}">
            ...
            <table>
                    ...
                    <tr>
                        ...
                        <td>${order.price}</td>
                        <td><a href="${basePath}orderServlet?action=showOrderDetail&orderId=${order.orderId}&totalPrice=${order.price}">訂單明細</a></td>
                        <td><a href="${basePath}orderServlet?action=receiveOrder&orderId=${order.orderId}">確認收貨</a></td>
                    </tr>
            </table>
            </c:forEach>
            <%--        靜態包含分頁條--%>
